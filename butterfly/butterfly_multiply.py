...
def deform_butterfly_mult_torch(input, num_mat, R_parameters, R_shapes, return_intermediates=False):
    ...
    # print('defrom:',output.shape)
    intermediates = [output]
    temp_p = 0
    for m in range(num_mat):
        # print('This is {} matrix'.format(m))
        R_shape = R_shapes[m]
        ...
        temp_p += num_p
        # output = butterfly_mult_single(output, R_parameters[m], R_shapes[m])
        intermediates.append(output)
    
        # print(output)
    return output.view(batch_size, output_size) if not return_intermediates else intermediates#if not return_intermediates else torch.stack([intermediate.view(batch_size, n) for intermediate in intermediates])

def butterfly_mult_single(output, R_parameter, R_shape):
    batch_size, n = output.shape[:2]
    output_size, input_size, row, col, diag = R_shape[:]
    assert n == input_size
    
    t = R_parameter.view(input_size // (col * diag), diag, row, col).permute(0, 2, 3, 1)
    # t = t.to(device = 'cuda')
    output_reshape = output.view(batch_size, n // (col * diag), 1, col, diag)
    # print(t)
    # print(output_reshape)
    output = (t * output_reshape).sum(dim=3)
    # print(output_size)
    
    return output.view(batch_size, output_size)
