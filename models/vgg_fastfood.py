...
from fastfood.fastfood_conv import Fastfood_2dConv
import torch.nn.functional as F
import math

device = 'cuda' if torch.cuda.is_available() else 'cpu'

__all__ = ['vgg16_bn_fastfood']

class VGG_Fastfood(nn.Module):

    def __init__(self, num_classes=100):
        super(VGG_Fastfood, self).__init__()
        ...
        self.classifier = FastfoodTransform(512, num_classes)
    ...
    def _make_layers_0():
        layers = []
        conv2d_1 = Fastfood_2dConv(3, 64, 3, padding=1, bias=True) 
        conv2d_2 = Fastfood_2dConv(64, 64, 3, padding=1, bias=True) 
    ...
    def _make_layers_1():
        layers = []
        conv2d_1 = Fastfood_2dConv(64, 128, 3, padding=1, bias=True) 
        conv2d_2 = Fastfood_2dConv(128, 128, 3, padding=1, bias=True) 
    ...
    def _make_layers_2():
        layers = []
        conv2d_1 = Fastfood_2dConv(128, 256, 3, padding=1, bias=True) 
        conv2d_2 = Fastfood_2dConv(256, 256, 3, padding=1, bias=True) 
        conv2d_3 = Fastfood_2dConv(256, 256, 3, padding=1, bias=True) 
    ...
    def _make_layers_3():
        layers = []
        conv2d_1 = Fastfood_2dConv(256, 512, 3, padding=1, bias=True) 
        conv2d_2 = Fastfood_2dConv(512, 512, 3, padding=1, bias=True) 
        conv2d_3 = Fastfood_2dConv(512, 512, 3, padding=1, bias=True) 
    ...
    def _make_layers_4():
        layers = []
        conv2d_1 = Fastfood_2dConv(512, 512, 3, padding=1, bias=True) 
        conv2d_2 = Fastfood_2dConv(512, 512, 3, padding=1, bias=True) 
        conv2d_3 = Fastfood_2dConv(512, 512, 3, padding=1, bias=True) 
        layers = [conv2d_1, nn.BatchNorm2d(512), nn.ReLU(inplace=True), conv2d_2, nn.BatchNorm2d(512), nn.ReLU(inplace=True), conv2d_3, nn.BatchNorm2d(512)]
        return nn.Sequential(*layers)
...
def vgg16_bn_fastfood(num_classes=100):
    return VGG_Fastfood(num_classes=num_classes)

if __name__ == '__main__':
    model = vgg16_bn_fastfood(num_classes=100)
    print(model)
    print('# vgg16_bn_fastfood parameters:', sum(param.numel() for param in model.parameters()))
