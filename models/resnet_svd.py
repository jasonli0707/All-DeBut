...
__all__ = ['resnet110_svd']


def svd_conv3x3(in_planes, out_planes, stride=1):
    """3x3 convolution with padding"""
    return SVD_2dConv(in_planes, out_planes, 3, stride=stride, padding=1, bias=False, compression_rate=0.1)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_last=False):
        ...
        self.conv1 = svd_conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = svd_conv3x3(planes, planes)
...
class ResNet110_SVD(nn.Module):

    def __init__(self, depth, num_filters, num_classes=100):
        super(ResNet110_SVD, self).__init__()
        """
        depth: 110
        ...
        self.conv1 = SVD_2dConv(3, num_filters[0], kernel_size=3, padding=1, bias=False, compression_rate=0.1)
        ...
        self.fc = SVD_Linear(nn.Linear(64, num_classes), compression_rate=0.1)
    ...
    def _make_layer_2(self, block, blocks):
        ...
        downsample = nn.Sequential(
            SVD_2dConv(16, 32, kernel_size=1, stride=2, bias=False, compression_rate=0.1),
            nn.BatchNorm2d(32)
        )
        layers = list([])
        # [32, 144]
    ...
    def _make_layer_3(self, block, blocks):
        # [64, 32]
        downsample = nn.Sequential(
            SVD_2dConv(32, 64, kernel_size=1, stride=2, bias=False, compression_rate=0.1),
            nn.BatchNorm2d(64)
        )
        layers = list([])
        #[64, 288]
...
def resnet110_svd(num_classes=100):
    return ResNet110_SVD(110, [16, 16, 32, 64], num_classes=num_classes)


if __name__ == '__main__':
    import torch
    x = torch.randn(2, 3, 32, 32) 
    net = resnet110_svd(num_classes=100)
