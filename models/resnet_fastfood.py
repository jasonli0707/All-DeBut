...
__all__ = ['resnet110_fastfood']


def fastfood_conv3x3(in_planes, out_planes, stride=1):
    """3x3 convolution with padding"""
    return Fastfood_2dConv(in_planes, out_planes, 3, stride=stride, padding=1, bias=False)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, is_last=False):
        ...
        self.conv1 = fastfood_conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = fastfood_conv3x3(planes, planes)
...
class ResNet110_Fastfood(nn.Module):

    def __init__(self, depth, num_filters, num_classes=100):
        super(ResNet110_Fastfood, self).__init__()
        """
        depth: 110
        ...
        self.conv1 = Fastfood_2dConv(3, num_filters[0], kernel_size=3, padding=1, bias=False)
        ...
        self.fc = FastfoodTransform(64, num_classes)
    ...
    def _make_layer_2(self, block, blocks):
        ...
        downsample = nn.Sequential(
            Fastfood_2dConv(16, 32, kernel_size=1, stride=2, bias=False),
            nn.BatchNorm2d(32)
        )
        layers = list([])
        # [32, 144]
    ...
    def _make_layer_3(self, block, blocks):
        # [64, 32]
        downsample = nn.Sequential(
            Fastfood_2dConv(32, 64, kernel_size=1, stride=2, bias=False),
            nn.BatchNorm2d(64)
        )
        layers = list([])
        #[64, 288]
...
def resnet110_fastfood(num_classes=100):
    return ResNet110_Fastfood(110, [16, 16, 32, 64], num_classes=num_classes)


if __name__ == '__main__':
    import torch
    x = torch.randn(2, 3, 32, 32) 
    net = resnet110_fastfood(num_classes=100)
