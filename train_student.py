...
def parse_option():
    ...
    # model
    parser.add_argument('--model_s', type=str, default='resnet110_debut',
    ...
    opt.model_name = 'S:{}_T:{}_{}_{}__epoch_{}_batch_{}_lr:{}_decay:{}_r:{}_a:{}_b:{}_trial_{}'.format(opt.model_s, opt.model_t, opt.dataset, opt.distill, opt.epochs, opt.batch_size
                                                                    ,opt.learning_rate, opt.weight_decay, opt.gamma, opt.alpha, opt.beta, opt.trial)


    if opt.model_s.split('_')[-1] == 'auto':
        opt.model_name = "shrinking_" + str(opt.shrinking_level) + '_' + opt.model_name
...
def main():
    ...
    if opt.model_s.split('_')[-1] == 'debut':
        R_shapes = process_chain_file(opt.r_shape_txt)
        model_s = model_dict[opt.model_s](R_shapes, num_classes=n_cls).to(device)
    elif opt.model_s.split('_')[-1] == 'auto':
        model_s = model_dict[opt.model_s](num_classes=n_cls, shrinking_level=opt.shrinking_level).to(device)
    else:
        model_s = model_dict[opt.model_s](num_classes=n_cls).to(device)
    
    print(model_s)
    print("Num of parameters = " + str(sum(param.numel() for param in model_s.parameters())))
